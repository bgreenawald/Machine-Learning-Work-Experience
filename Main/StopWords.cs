...
namespace Membership_Application
{
    ...
    public class StopWords
    {
        ...
        public StopWords()
        {
            this.words = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                "let's","me","more","most","mustn't","my","myself","n't", "no","nor","not","of","off","on","once","only","or","other",
                ...
                "wouldn't","you","you'd","you'll","you're","you've","your","yours","yourself","yourselves", "within",
                ".", ",", "'", "\"", "\" ", "''", "`", "``", "<", ">", "~",  ";", ":", "'s", "'d", "j", "ii", "-lrb-", "-rrb-", "!", "@", "#", "$", "%",
                "^", "&", "*", "(", ")", "-", "?","", "s"
            };
        }
        ...
        {
            words = FilterNums(words);
            return words.Where(a => !this.words.Contains(a));
        }

        public IEnumerable<string> FilterNums(IEnumerable<string> words_with_nums)
        {
            List<string> no_nums = new List<string>();
            foreach(string word in words_with_nums)
            {
                double result;
                int int_result;
                ...
                {
                    no_nums.Add(word);
                }else if(word[0] != '$' && int.TryParse(word[0] + "", out int_result))
                {
                    no_nums.Add(word);
                }
            }
            return (IEnumerable<string>) no_nums;
        }
    }
